#include <Arduino.h>
//////////// НАСТРОЙКИ ////////////
#define Version_Firmware "Serega88kos/Clock_ESP8266_WS2812_IOT@1.0"  // версия прошивки
...
struct Wifi {
  ...
  uint8_t LEDS_IN_SEGMENT = 4;   // задаём сколько у нас светодиодов в сегменте
  uint8_t DOTS_NUM = 2;          // задаём сколько у нас разделительных точек 2/4/6/8
  uint8_t DOT_TEMP = 0;          // включаем точку десятых для температуры, 0 или 1
};
Wifi wifi;
...
struct Other {
  ...
  bool min_max = false;
  bool night_mode = 0;
  uint8_t start_night = 0;
  uint8_t stop_night = 0;
  int night_bright = 10;
};
Other other;
