    ...
    case 0:
      {
        ...
        hub.WidgetSize(33);
        flag_wifi |= hub.Spinner(F("LEDS_IN_SEGMENT"), &wifi.LEDS_IN_SEGMENT, GH_INT16, F("Точек в сегм"), 1, 5, 1);
        flag_wifi |= hub.Spinner(F("DOTS_NUM"), &wifi.DOTS_NUM, GH_INT16, F("Разд. точек"), 1, 8, 1);
        flag_wifi |= hub.Spinner(F("DOT_TEMP"), &wifi.DOT_TEMP, GH_INT16, F("Точка для темп."), 0, 1, 1);
        if (flag_wifi) {
          wifi_.update();
        }
      }
