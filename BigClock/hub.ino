    ...
    case 4:
      {
        ...
        {
          gh::Row r(b);
          b.Select(&dfp.board).text("DFPLAYER_MINI;DFPLAYER_FN_X10P;DFPLAYER_HW_247A;DFPLAYER_NO_CHECKSUM").label("Тип платы").attach(&flag_dfp);
        }
        {
          gh::Row r(b);
          ...
          if (dfp.status_kuku) {
            ...
            b.Button_("btnTest").label("Тест [выкл]").fontSize(20).icon("f04b").disabled();
          }
        ...
        b.Title(F("Режимы")).fontSize(20);
        {
          ...
          b.Spinner(&dfp.grom_mp3).label("Громкость").fontSize(15).range(0, 30, 1).attach(&flag_dfp_vol);
        }
        if (flag_dfp) {
          _dfp.update();
        }
